################################################################################
# PAI Diagnostic Tool
# Function & Implementation Sample Using Johnson/Strother Mood
################################################################################

################################################################################
# Load Packages
################################################################################
library(dplyr); library(tidyr); library(gridExtra); library(ggplot2); library(grid); library(gridtext); library(cowplot); library(stringr)

################################################################################
# Diagnostic Tool Function -- Run Full Block
# pai_object = Output object from PAI Process
# variables = Can declare specific variables (or Null for ALL)
# bins = Bins along x-axis to declare (Default = 5)
# bin_cut = Cuts Along x-axis to create bins (Default = 5)
################################################################################
...
################################################################################
# Sample Implementation w/ Fortas
# Assuming PAI Folder w/ Justice-level Objects Stored Locally
# Note: Likely to Raise Warnings -- Ignore Them -- They're General Warnings from GridText
## That I Am Working to Suppress.
################################################################################

dropbox <- list.files("E:/PAI", full.names = T) # List Files From Dropbox (Justice-Level)
fortas <- list.files(dropbox[1], full.names = T) # Example Using Fortas
fortas_ada <- get(load(fortas[1])) #Load Fortas 'adaboost' Data
fortas_ada_test <- pai_diagnostic(fortas_ada) #Runs Diagnostic on Fortas 'adaboost'
test$figures$placebo #Prints Placebo Iterations Figure w/ Mood, Issue, Etc.
