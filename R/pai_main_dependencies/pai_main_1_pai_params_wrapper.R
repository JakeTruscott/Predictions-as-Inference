pai_params_wrapper <- function(data, model, outcome, predictors, interactions, drop_vars, save_drop_var_models, cores, placebo_iterations, folds, train_split, drop_sparse_vars, sparse_variable_threshold, custom_tc, outcome_type, assign_factors, list_drop_vars, seed){
...

  {
    ...
    if (!is.null(sparse_check['factors'])){
      factors <- unname(unlist(sparse_check['factors']))
    }
